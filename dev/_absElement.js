...
function Element() {
  this.initialize = function () {
    ...
    if (null != this.propertiesStack && 0 != this.propertiesStack.length) {
      (function () {
        var _self = _this,
            res = _this.propertiesStack.pop();
        _this.serializedProperties.forEach(function (resProperty) {
    ...
    return this.serializedProperties.forEach(function (val, index) {
      var pro = _self[val];
      'string' == typeof pro && (pro = '"' + pro + '"'), result += '"' + val + '":' + pro, len > index + 1 && (result += ',');
    }), result += '}';
  };
}
...
CanvasRenderingContext2D.prototype.AmostRoundRect = function (x, y, width, height, borderRadius) {
  'undefined' == typeof borderRadius && (borderRadius = 5), this.beginPath(), this.moveTo(x + borderRadius, y), this.lineTo(x + width - borderRadius, y), this.quadraticCurveTo(x + width, y, x + width, y + borderRadius), this.lineTo(x + width, y + height - borderRadius), this.quadraticCurveTo(x + width, y + height, x + width - borderRadius, y + height), this.lineTo(x + borderRadius, y + height), this.quadraticCurveTo(x, y + height, x, y + height - borderRadius), this.lineTo(x, y + borderRadius), this.quadraticCurveTo(x, y, x + borderRadius, y), this.closePath();
};
...
CanvasRenderingContext2D.prototype.AmostDashedLineTo = function (x, y, width, height, dashedPattern) {
  ...
  var distanceW = width - x,
      distanceH = height - y,
      hypotenuse = Math.floor(Math.sqrt(distanceW * distanceW + distanceH * distanceH)),
      // 平方根,计算斜边
...
//# sourceMappingURL=_absElement.js.map