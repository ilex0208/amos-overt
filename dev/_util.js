...
function MessageBus(name) {
  var _self = this;
  this.name = name, this.messageMap = {}, this.messageCount = 0, this.subscribe = function (topic, action) {
    var actions = _self.messageMap[topic];
    null == actions && (_self.messageMap[topic] = []), _self.messageMap[topic].push(action), _self.messageCount++;
...
function getDistance(p1, p2, x, y) {
  var dx = void 0,
      dy = void 0;
  return null == x && null == y ? (dx = p2.x - p1.x, dy = p2.y - p1.y) : (dx = x - p1, dy = y - p2), Math.sqrt(dx * dx + dy * dy);
}
...
function mouseCoords(ev) {
  return ev = cloneEvent(ev), ev.pageX || (ev.pageX = ev.clientX + document.body.scrollLeft - document.body.clientLeft, ev.pageY = ev.clientY + document.body.scrollTop - document.body.clientTop), ev;
}
...
function rotatePoint(bx, by, x, y, angle) {
  ...
  // 方位角加上旋转角度(弧度)
  var azimuth = Math.atan2(dy, dx) + angle;
  return {
...
function isPointInLine(point, path1, path2) {
  var pathDis = getDistance(path1, path2),
...
function getProperties(obj, arr) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    var val = obj[arr[i]];
    'string' == typeof val ? val = '"' + val + '"' : void 0 == val && (val = null), result += arr[i] + ':' + val;
  }
  return result;
}
...
function toJson(stage) {
  var sceneProperties = 'backgroundColor,visible,mode,rotate,alpha,scaleX,scaleY,shadow,translateX,translateY,areaSelect,paintAll'.split(','),
      ...
      result = '{';
  result += 'frames:' + stage.frames, result += ', scenes:[';
  for (var i = 0; i < stage.childs.length; i++) {
    var sceneItem = stage.childs[i];
    result += '{', result += getProperties(sceneItem, sceneProperties), result += ', elements:[';
    for (var j = 0; j < sceneItem.childs.length; j++) {
      var eleItem = sceneItem.childs[j];
      j > 0 && (result += ','), result += '{', result += getProperties(eleItem, eleProperties), result += '}';
    }
    result += ']}';
  }
  result += ']'; // scenes结束符
...
function generateColor(ctx, img, rgba) {
  ...
  var tmpW = targetCanvas.width = img.width,
      tmpH = targetCanvas.height = img.height;
  ctx.clearRect(0, 0, targetCanvas.width, targetCanvas.height), ctx.drawImage(img, 0, 0);
  // 返回 ImageData 对象，该对象为画布上指定的矩形复制像素数据
  var imgData = ctx.getImageData(0, 0, img.width, img.height);
  // 返回Uint8ClampedArray, RGBA, 0到255
  var data = imgData.data;
  for (var i = 0; i < tmpW; i++) {
    for (var j = 0; j < tmpH; j++) {
      // 构建数据索引:imagedata读取的像素数据存储在data属性里，是从上到下，从左到右的，每个像素需要占用4位数据，分别是r,g,b,alpha透明通道
      var dataIndex = 4 * (i + j * tmpW);
      0 != data[dataIndex + 3] && (null != rgbR && (data[dataIndex + 0] += rgbR), null != rgbG && (data[dataIndex + 1] += rgbG), null != rgbA && (data[dataIndex + 2] += rgbA));
    }
  }
...
function changeColorByOpts(ctx, img, alarmOps) {
  var _alarmOps = Object.assign(alarmDefaulOps, alarmOps);
  var rgbR = _alarmOps.rgbR;
  var rgbG = _alarmOps.rgbG;
  var rgbB = _alarmOps.rgbB;
  var rgbA = _alarmOps.rgbA;
  targetCanvas.width = img.width, targetCanvas.height = img.height;
  ctx.clearRect(0, 0, targetCanvas.width, targetCanvas.height), ctx.drawImage(img, 0, 0);
  // 返回 ImageData 对象，该对象为画布上指定的矩形复制像素数据
  var imgData = ctx.getImageData(0, 0, img.width, img.height);
  // 返回Uint8ClampedArray, RGBA, 0到255
...
function genImageAlarmByOpts(img, alarmImgOps) {
  var _ops = Object.assign(alarmDefaulOps, alarmImgOps);
  try {
    if (alarmImageCache[img.src]) {
      return alarmImageCache[img.src];
    } else {
...
function getOffsetPosition(domEle) {
  ...
  if ('getBoundingClientRect' in document.documentElement) {
    var clientRect = domEle.getBoundingClientRect(),
    ...
    _top = clientRect.top + (self.pageYOffset || _de && _de.scrollTop || _body.scrollTop) - tempTop, _left = clientRect.left + (self.pageXOffset || _de && _de.scrollLeft || _body.scrollLeft) - tempLeft;
  } else {
    do {
      _top += domEle.offsetTop || 0, _left += domEle.offsetLeft || 0, domEle = domEle.offsetParent;
    } while (domEle);
  }
  return {
    left: _left,
...
function lineF(x1, y1, x2, y2) {
  ...
  CalcLine.k = tmpYdividedX, CalcLine.b = tmpY, CalcLine.x1 = x1, CalcLine.x2 = x2, CalcLine.y1 = y1, CalcLine.y2 = y2;
  return CalcLine;
}
...
function intersection(calcLine1, calcLine2) {
  var $x = void 0,
      $y = void 0;
  return calcLine1.k == calcLine2.k ? null : (1 / 0 == calcLine1.k || calcLine1.k == -1 / 0 ? ($x = calcLine1.x1, $y = calcLine2(calcLine1.x1)) : 1 / 0 == calcLine2.k || calcLine2.k == -1 / 0 ? ($x = calcLine2.x1, $y = calcLine1(calcLine2.x1)) : ($x = (calcLine2.b - calcLine1.b) / (calcLine1.k - calcLine2.k), $y = calcLine1($x)), 0 == isPointInLineSeg($x, $y, calcLine1) ? null : 0 == isPointInLineSeg($x, $y, calcLine2) ? null : { x: $x, y: $y });
}
...
function intersectionLineBound(calcLine, bound) {
  // 临时CalcLine
  var tempCalcLine = lineF(bound.left, bound.top, bound.left, bound.bottom);
  // 交叉点
  var resultBound = intersection(calcLine, tempCalcLine);
  return null == resultBound && (tempCalcLine = lineF(bound.left, bound.top, bound.right, bound.top), resultBound = intersection(calcLine, tempCalcLine), null == resultBound && (tempCalcLine = lineF(bound.right, bound.top, bound.right, bound.bottom), resultBound = intersection(calcLine, tempCalcLine), null == resultBound && (tempCalcLine = lineF(bound.left, bound.bottom, bound.right, bound.bottom), resultBound = intersection(calcLine, tempCalcLine)))), resultBound;
}

// requestAnim shim layer by ilex
requestAnimationFrame = function () {
  return window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame || window.oRequestAnimationFrame || function ( /* function */callback, /* DOMElement */element) {
