...
var SceneMode = Tobj.SceneMode;
var MouseCursor = Tobj.MouseCursor;
...
function Scene(stage) {
  ...
  function createRect(x, y, width, height) {
    return function (ctx) {
      ctx.beginPath(), ctx.strokeStyle = 'rgba(0,0,236,0.5)', ctx.fillStyle = 'rgba(0,0,236,0.1)', ctx.rect(x, y, width, height), ctx.fill(), ctx.stroke(), ctx.closePath();
    };
  }
  var _self = this;
  this.initialize = function () {
    Scene.prototype.initialize.apply(this, arguments), this.messageBus = new Util.MessageBus(), this.elementType = 'scene', this.childs = [], this.zIndexMap = {}, this.zIndexArray = [], this.backgroundColor = '255,255,255', this.visible = true, this.alpha = 0, this.scaleX = 1, this.scaleY = 1, this.mode = SceneMode.normal, this.translate = true, this.translateX = 0, this.translateY = 0, this.lastTranslateX = 0, this.lastTranslateY = 0, this.mouseDown = false, this.mouseDownX = null, this.mouseDownY = null, this.mouseDownEvent = null, this.areaSelect = true, this.operations = [], this.selectedElements = [], this.paintAll = false;
  ...
  }, this.initialize(), this.setBackground = function (bg) {
    this.background = bg;
  }, this.addTo = function (st) {
    this.stage !== st && null != st && (this.stage = st);
  }, null != stage && (stage.add(this), this.addTo(stage)), this.show = function () {
    this.visible = true;
  }, this.hide = function () {
    this.visible = false;
  }, this.paint = function (ctx) {
    if (0 != this.visible && null != this.stage) {
      ctx.save(), this.paintBackgroud(ctx), ctx.restore(), ctx.save(), ctx.scale(this.scaleX, this.scaleY);
      if (1 == this.translate) {
        var offT = this.getOffsetTranslate(ctx);
        ctx.translate(offT.translateX, offT.translateY);
      }
      this.paintChilds(ctx), ctx.restore(), ctx.save(), this.paintOperations(ctx, this.operations), ctx.restore();
    }
    ...
    if (null != this.background) {
      ...
      ctx.beginPath(), ctx.fillStyle = 'rgba(' + this.backgroundColor + ',' + this.alpha + ')', ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height), ctx.closePath();
    }
  }, this.getDisplayedElements = function () {
    var eles = [];
    for (var i = 0; i < this.zIndexArray.length; i++) {
      var temp = this.zIndexArray[i];
      var maps = this.zIndexMap[temp];
    ...
    for (var i = 0; i < this.zIndexArray.length; i++) {
      var zIarr = this.zIndexArray[i],
          value = this.zIndexMap[zIarr];
      for (var j = 0; j < value.length; j++) {
        ...
        if (true === this.paintAll || this.isVisiable(ele)) {
          ...
          if (true === ele.transformAble) {
            ...
            ctx.translate(h.x, h.y), ele.rotate && ctx.rotate(ele.rotate), ele.scaleX && ele.scaleY ? ctx.scale(ele.scaleX, ele.scaleY) : ele.scaleX ? ctx.scale(ele.scaleX, 1) : ele.scaleY && ctx.scale(1, ele.scaleY);
          }
          if (true === ele.shadow) {
    ...
    var offTrans = this.getOffsetTranslate(),
        calcX = ele.x + offTrans.translateX,
        calcY = ele.y + offTrans.translateY;
    calcX *= this.scaleX, calcY *= this.scaleY;
    var _width = calcX + ele.width * this.scaleX,
  ...
  */
  this.getElementsByClass = function (targetEle) {
    ...
    return this.operations.push(opFn), this;
    ...
    this.childs.push(eleObj), null == this.zIndexMap[eleObj.zIndex] && (this.zIndexMap[eleObj.zIndex] = [], this.zIndexArray.push(eleObj.zIndex), this.zIndexArray.sort(function (a, b) {
      return a - b;
  ...
  this.remove = function (eleObj) {
    ...
    var val = this.zIndexMap[eleObj.zIndex];
    val && (this.zIndexMap[eleObj.zIndex] = Util.removeFromArray(val, eleObj)), eleObj.removeHandler(this);
  },
  /**
   * 清空画布中的所有元素
   * @param {any} eleObj 要被清除的元素
  ...
  this.removeFromSelected = function (eleObj) {
    for (var i = 0; i < this.selectedElements.length; i++) {
      var selectObj = this.selectedElements[i];
      eleObj === selectObj && (this.selectedElements = this.selectedElements.del(i));
    }
  }, this.toSceneEvent = function (ev) {
    var event = Util.clone(ev);
    event.x /= this.scaleX;
    event.y /= this.scaleY;
    if (1 == this.translate) {
      var offTrans = this.getOffsetTranslate();
      event.x -= offTrans.translateX, event.y -= offTrans.translateY;
    }
    return null != event.dx && (event.dx /= this.scaleX, event.dy /= this.scaleY), null != this.currentElement && (event.target = this.currentElement), event.scene = this, event;
  }, this.selectElement = function (ev) {
    var targetEle = _self.getElementByXY(ev.x, ev.y);
    if (null != targetEle) {
      ev.target = targetEle, targetEle.mousedownHander(ev), targetEle.selectedHandler(ev);
      if (_self.notInSelectedNodes(targetEle)) {
        ev.ctrlKey || _self.cancleAllSelected(), _self.addToSelected(targetEle);
      } else {
        1 == ev.ctrlKey && (targetEle.unselectedHandler(), this.removeFromSelected(targetEle));
        for (var i = 0; i < this.selectedElements.length; i++) {
          var selectEle = this.selectedElements[i];
    ...
    var tempEvent = this.toSceneEvent(ev);
    this.mouseDown = true, this.mouseDownX = tempEvent.x, this.mouseDownY = tempEvent.y, this.mouseDownEvent = tempEvent;
    if (this.mode == SceneMode.normal) {
      this.selectElement(tempEvent), (null == this.currentElement || this.currentElement instanceof Link) && 1 == this.translate && (this.lastTranslateX = this.translateX, this.lastTranslateY = this.translateY);
    } else {
    ...
    this.stage.cursor != MouseCursor.normal && (this.stage.cursor = MouseCursor.normal), _self.clearOperations();
    var tempEvent = this.toSceneEvent(ev);
    null != this.currentElement && (tempEvent.target = _self.currentElement, this.currentElement.mouseupHandler(tempEvent)), this.dispatchEvent('mouseup', tempEvent), this.mouseDown = false;
  }, this.dragElements = function (ev) {
    if (null != this.currentElement && 1 == this.currentElement.dragable) {
      for (var i = 0; i < this.selectedElements.length; i++) {
        var tempEle = this.selectedElements[i];
        if (0 != tempEle.dragable) {
          var tempEvent = Util.clone(ev);
          tempEvent.target = tempEle, tempEle.mousedragHandler(tempEvent);
        }
      }
    }
  }, this.mousedragHandler = function (ev) {
    var tempEvent = this.toSceneEvent(ev);
    this.mode == SceneMode.normal ? null == this.currentElement || this.currentElement instanceof Link ? 1 == this.translate && (this.stage.cursor = MouseCursor.closed_hand, this.translateX = this.lastTranslateX + tempEvent.dx, this.translateY = this.lastTranslateY + tempEvent.dy) : this.dragElements(tempEvent) : this.mode == SceneMode.drag ? 1 == this.translate && (this.stage.cursor = MouseCursor.closed_hand, this.translateX = this.lastTranslateX + tempEvent.dx, this.translateY = this.lastTranslateY + tempEvent.dy) : this.mode == SceneMode.select ? null != this.currentElement ? 1 == this.currentElement.dragable && this.dragElements(tempEvent) : 1 == this.areaSelect && this.areaSelectHandle(tempEvent) : this.mode == SceneMode.edit && (null == this.currentElement || this.currentElement instanceof Link ? 1 == this.translate && (this.stage.cursor = MouseCursor.closed_hand, this.translateX = this.lastTranslateX + tempEvent.dx, this.translateY = this.lastTranslateY + tempEvent.dy) : this.dragElements(tempEvent)), this.dispatchEvent('mousedrag', tempEvent);
  }, this.areaSelectHandle = function (ev) {
    var offL = ev.offsetLeft,
        offT = ev.offsetTop,
    ...
    _self.clearOperations().addOperation(targetRect), offL = ev.x, offT = ev.y, deOffL = this.mouseDownEvent.x, deOffTop = this.mouseDownEvent.y, targetOffL = offL >= deOffL ? deOffL : offL, targetOffT = offT >= deOffTop ? deOffTop : offT, tarScaleX = Math.abs(ev.dx), tarScaleY = Math.abs(ev.dy);
    var _width = targetOffL + tarScaleX,
        _height = targetOffT + tarScaleY;
    for (var i = 0; i < _self.childs.length; i++) {
      var childItem = _self.childs[i];
    ...
    var c = this.toSceneEvent(ev);
    if (this.mode == SceneMode.drag) {
      return void (this.stage.cursor = MouseCursor.open_hand);
    }
    ...
    var d = _self.getElementByXY(c.x, c.y);
    ...
    var b = this.toSceneEvent(ev);
    this.currentElement ? (b.target = this.currentElement, this.currentElement.dbclickHandler(b)) : _self.cancleAllSelected(), this.dispatchEvent('dbclick', b);
  ...
  return events.forEach(function (a) {
    ...
    this.serializedProperties.forEach(function (c) {
      var item = a[c];
      'background' == c && (item = a._background.src), 'string' == typeof item && (item = '"' + item + '"'), b += '"' + c + '":' + item + ',';
    }), b += '"childs":[';
    ...
    set: function set(imgUrl) {
      ...
      if ('string' == typeof imgUrl) {
        (function () {
          ...
          }), _this._background = img;
        })();
      } else {
        this._background = imgUrl;
      }
    }
  }
});
...
//# sourceMappingURL=_scene.js.map