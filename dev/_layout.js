...
function circleLayoutNodes(elementArray, animates) {
  ...
  animates.beginAngle || 0, animates.endAngle || 2 * Math.PI;
  if (null == $cx || null == $cy) {
    var centerPos = getNodesCenter(elementArray);
  ...
  elementArray.forEach(function (item) {
    null == animates.nodeDiameter ? (item.diameter && (dia = item.diameter), dia = item.radius ? 2 * item.radius : Math.sqrt(2 * item.width * item.height), diaArray.push(dia)) : diaArray.push(dia), totalDia += dia;
  }), elementArray.forEach(function (value, index) {
    var singleDia = diaArray[index] / totalDia;
    circumferenceArray.push(Math.PI * singleDia);
  });
  var twoCircum = (elementArray.length, circumferenceArray[0] + circumferenceArray[1]),
      twoRadius = diaArray[0] / 2 + diaArray[1] / 2,
      tempR = twoRadius / 2 / Math.sin(twoCircum / 2);
  null != minR && minR > tempR && (tempR = minR);
  var _radius = tempR * _hScale,
      _radiusB = tempR * _vScale,
      singleAnimate = animates.animate;
  var angle = 0;
  if (singleAnimate) {
    (function () {
      var millsec = singleAnimate.time || 1000;
      // v = 0;
      elementArray.forEach(function (item, index) {
        ...
        var tempX = $cx + Math.cos(angle) * _radius,
    ...
    elementArray.forEach(function (item, index) {
      angle += 0 == index ? circumferenceArray[index] : circumferenceArray[index - 1] + circumferenceArray[index];
      var tempCX = $cx + Math.cos(angle) * _radius,
          tempCY = $cy + Math.sin(angle) * _radiusB;
      item.cx = tempCX, item.cy = tempCY;
    });
  }
  return {
    cx: $cx,
...
function GridLayout(row, column) {
  return function (container) {
    var elementArray = container.childs;
...
function FlowLayout(horizontal, vertical) {
  return null == horizontal && (horizontal = 0), null == vertical && (vertical = 0), function (container) {
    var elementArray = container.childs;
    if (!(elementArray.length <= 0)) {
      var bound = container.getBound(),
          _left = bound.left,
          _top = bound.top;
      for (var h = 0; h < elementArray.length; h++) {
        var ele = elementArray[h];
        _left + ele.width >= bound.right && (_left = bound.left, _top += vertical + ele.height), ele.setLocation(_left, _top), _left += horizontal + ele.width;
      }
    }
  };
}
  ...
  return function (container, elementArray) {
    if (elementArray.length > 0) {
      ...
      for (var i = 0; i < elementArray.length; i++) {
        ...
        ele.x <= _x && (_x = ele.x), ele.x >= reverseX && (reverseX = ele.x), ele.y <= _y && (_y = ele.y), ele.y >= reverseY && (reverseY = ele.y), _width = reverseX - _x + ele.width, _height = reverseY - _y + ele.height;
      }
      container.x = _x, container.y = _y, container.width = _width, container.height = _height;
    }
  };
}
...
function calcSize(eleArr) {
  ...
  return eleArr.forEach(function (ele) {
    totalW += ele.width, totalH += ele.height;
  }), {
...
function calcAndSetPosition(scene, rootNode, _x, _y) {
  rootNode.x += _x, rootNode.y += _y;
  var allChilds = getNodeChilds(scene, rootNode);
...
function getTreeNodes(scene, rootNode) {
  function calcNodes(node, index) {
    var allChilds = getNodeChilds(scene, node);
    null == result[index] && (result[index] = {}, result[index].nodes = [], result[index].childs = []), result[index].nodes.push(node), result[index].childs.push(allChilds);
    for (var i = 0; i < allChilds.length; i++) {
      calcNodes(allChilds[i], index + 1), allChilds[i].parent = node;
    }
  }
  var result = [];
  return calcNodes(rootNode, 0), result;
}
...
function TreeLayout(directionType, _width, _height) {
  return function ($stage) {
    function executeTree(scene, rootNode) {
      var deep = getTreeDeep(scene, rootNode),
          tempRns = getTreeNodes(scene, rootNode),
          nodeArr = tempRns['' + deep].nodes;
      for (var i = 0; i < nodeArr.length; i++) {
        var clNode = nodeArr[i],
            tempW = (i + 1) * (_width + 10),
            tempH = deep * _height;
        'down' == directionType || ('up' == directionType ? tempH = -tempH : 'left' == directionType ? (tempW = -deep * _height, tempH = (i + 1) * (_width + 10)) : 'right' == directionType && (tempW = deep * _height, tempH = (i + 1) * (_width + 10))), clNode.setLocation(tempW, tempH);
      }
      for (var j = deep - 1; j >= 0; j--) {
        // 当前层的node集合
        var currentLevelNodes = tempRns['' + j].nodes,
            tempChilds = tempRns['' + j].childs;
        for (var k = 0; k < currentLevelNodes.length; k++) {
          var nodeItem = currentLevelNodes[k],
              childItem = tempChilds[k];
          'down' == directionType ? nodeItem.y = j * _height : 'up' == directionType ? nodeItem.y = -j * _height : 'left' == directionType ? nodeItem.x = -j * _height : 'right' == directionType && (nodeItem.x = j * _height), childItem.length > 0 ? 'down' == directionType || 'up' == directionType ? nodeItem.x = (childItem[0].x + childItem[childItem.length - 1].x) / 2 : ('left' == directionType || 'right' == directionType) && (nodeItem.y = (childItem[0].y + childItem[childItem.length - 1].y) / 2) : k > 0 && ('down' == directionType || 'up' == directionType ? nodeItem.x = currentLevelNodes[k - 1].x + currentLevelNodes[k - 1].width + _width : ('left' == directionType || 'right' == directionType) && (nodeItem.y = currentLevelNodes[k - 1].y + currentLevelNodes[k - 1].height + _width));
          if (k > 0) {
            if ('down' == directionType || 'up' == directionType) {
              if (nodeItem.x < currentLevelNodes[k - 1].x + currentLevelNodes[k - 1].width) {
                var totalW = currentLevelNodes[k - 1].x + currentLevelNodes[k - 1].width + _width,
                    tempX = Math.abs(totalW - nodeItem.x);
                for (var m = k; m < currentLevelNodes.length; m++) {
              ...
              var totalH = currentLevelNodes[k - 1].y + currentLevelNodes[k - 1].height + _width,
                  tempY = Math.abs(totalH - nodeItem.y);
              for (var n = k; n < currentLevelNodes.length; n++) {
    ...
    var singleSize = null;
    null == _width && (singleSize = calcSize($stage.childs), _width = singleSize.width, ('left' == directionType || 'right' == directionType) && (_width = singleSize.width + 10)), null == _height && (null == singleSize && (singleSize = calcSize($stage.childs)), _height = 2 * singleSize.height), null == directionType && (directionType = 'down');
    var rns = getRootNodes($stage.childs);
    if (rns.length > 0) {
      (function () {
        executeTree($stage.childs, rns[0]);
        var bound = Util.getElementsBound($stage.childs),
...
function CircleLayout(diameter) {
  return function ($scene) {
    function calcLocation(_scene, rootNode, dia) {
      var allChilds = getNodeChilds(_scene, rootNode);
      if (0 != allChilds.length) {
        (function () {
          null == dia && (dia = diameter);
          var girth = 2 * Math.PI / allChilds.length;
          allChilds.forEach(function (child, index) {
            var _x = rootNode.x + dia * Math.cos(girth * index),
    ...
    var nodes = getRootNodes($scene.childs);
    if (nodes.length > 0) {
      (function () {
        calcLocation($scene.childs, nodes[0]);
        var bound = Util.getElementsBound($scene.childs),
...
function getCyclePositions(x, y, counts, radius, begin, end) {
  var beginAngle = begin ? begin : 0,
      endAngle = end ? end : 2 * Math.PI,
      diff = endAngle - beginAngle,
      avg = diff / counts,
      results = [];
  beginAngle += avg / 2;
  for (var i = beginAngle; endAngle >= i; i += avg) {
    var _x = x + Math.cos(i) * radius,
...
function getTreePositions(x, y, count, horizontal, vertical, dir) {
  ...
  if (direction == 'bottom') {
    ...
    var bstart = x - count / 2 * horizontal + horizontal / 2;
    ...
    var tstart = x - count / 2 * horizontal + horizontal / 2;
    ...
    var rstart = y - count / 2 * horizontal + horizontal / 2;
    ...
    var lstart = y - count / 2 * horizontal + horizontal / 2;
...
function adjustPosition(ele, allChilds) {
  if (ele.layout) {
    var _layout = ele.layout,
        _type = _layout.type,
        pos = null;
    if ('circle' == _type) {
      var _radius = _layout.radius || Math.max(ele.width, ele.height);
      pos = getCyclePositions(ele.cx, ele.cy, allChilds.length, _radius, ele.layout.beginAngle, ele.layout.endAngle);
    } else if ('tree' == _type) {
      var horizontal = _layout.width || 50,
...
function layoutNode(scene, targetEle, isAutoLayout) {
  var allChild = getNodeChilds(scene.childs, targetEle);
...
function springLayout($rootNode, $scene) {
  ...
  function dynamicAdjustPos(rn, tarNode) {
    var distanceX = rn.x - tarNode.x,
        distanceY = rn.y - tarNode.y;
    currentX += distanceX * additive, currentY += distanceY * additive, currentX *= multiplicative, currentY *= multiplicative, currentY += diff, tarNode.x += currentX, tarNode.y += currentY;
  }
...
function getTreeDeep($scene, $rootNode) {
  function calcDeep(_scene, _rootNode, _deep) {
    var allchilds = getNodeChilds(_scene, _rootNode);
  ...
  var deep = 0;
  return calcDeep($scene, $rootNode, 0), deep;
}
