...
var _default = Constants.DEFAULT;
...
function DisplayElement() {
  this.initialize = function () {
    DisplayElement.prototype.initialize.apply(this, arguments), this.elementType = 'displayElement', this.x = 0, this.y = 0, this.width = 32, this.height = 32, this.visible = true, this.alpha = 1, this.rotate = 0, this.scaleX = 1, this.scaleY = 1, this.strokeColor = '22,124,255', this.borderColor = '22,124,255', this.fillColor = '22,124,255', this.shadow = false, this.shadowBlur = 5, this.shadowColor = 'rgba(0,0,0,0.5)', this.shadowOffsetX = 3, this.shadowOffsetY = 6, this.transformAble = false, this.zIndex = 0;
    var properties = 'x,y,width,height,visible,alpha,rotate,scaleX,scaleY,strokeColor,fillColor,shadow,shadowColor,shadowOffsetX,shadowOffsetY,transformAble,zIndex'.split(',');
    this.serializedProperties = this.serializedProperties.concat(properties);
  }, this.initialize(), this.paint = function (ctx) {
    ctx.beginPath(), ctx.fillStyle = 'rgba(' + this.fillColor + ',' + this.alpha + ')', ctx.rect(-this.width / 2, -this.height / 2, this.width, this.height), ctx.fill(), ctx.stroke(), ctx.closePath();
  ...
  this.setDimension = function (dimension) {
    var _dimension = dimension || _default;
    ...
    return this.setLocation(x, y), this.setSize(width, height), this;
...
function InteractiveElement() {
  this.initialize = function () {
    InteractiveElement.prototype.initialize.apply(this, arguments), this.elementType = 'interactiveElement', this.dragable = false, this.selected = false, this.showSelected = true, this.selectedLocation = null, this.isMouseOver = false;
    var properties = 'dragable,selected,showSelected,isMouseOver'.split(',');
    this.serializedProperties = this.serializedProperties.concat(properties);
  }, this.initialize(), this.paintSelected = function (ctx) {
    ...
    this.selected = true, this.selectedLocation = {
      x: this.x,
      y: this.y
    };
    ...
    this.isMouseOver = true, this.dispatchEvent('mouseover', msg);
  }, this.mousemoveHandler = function (msg) {
    this.dispatchEvent('mousemove', msg);
  }, this.mouseoutHandler = function (msg) {
    this.isMouseOver = false, this.dispatchEvent('mouseout', msg);
  }, this.mousedragHandler = function (msg) {
    var x = this.selectedLocation.x + msg.dx,
        y = this.selectedLocation.y + msg.dy;
    this.setLocation(x, y), this.dispatchEvent('mousedrag', msg);
  }, this.addEventListener = function (topic, fn) {
    var _self = this,
        action = function action(data) {
      fn.call(_self, data);
    };
    return this.messageBus || (this.messageBus = new Util.MessageBus()), this.messageBus.subscribe(topic, action), this;
...
function EditableElement() {
  this.initialize = function () {
    EditableElement.prototype.initialize.apply(this, arguments), this.editAble = false, this.selectedPoint = null;
    ...
    EditableElement.prototype.unselectedHandler.apply(this, arguments), this.selectedSize = null, this.editAble = false;
  };
  var points = ['Top_Left', 'Top_Center', 'Top_Right', 'Middle_Left', 'Middle_Right', 'Bottom_Left', 'Bottom_Center', 'Bottom_Right'];
  this.paintCtrl = function (ctx) {
    if (false !== this.editAble) {
      ctx.save();
      for (var i = 0; i < points.length; i++) {
        var ctrPos = this.getCtrlPosition(points[i]);
        ctrPos.left -= this.cx, ctrPos.right -= this.cx, ctrPos.top -= this.cy, ctrPos.bottom -= this.cy;
        var $width = ctrPos.right - ctrPos.left,
            $heith = ctrPos.bottom - ctrPos.top;
        ctx.beginPath(), ctx.strokeStyle = 'rgba(0,0,0,0.8)', ctx.rect(ctrPos.left, ctrPos.top, $width, $heith), ctx.stroke(), ctx.closePath(), ctx.beginPath(), ctx.strokeStyle = 'rgba(255,255,255,0.3)', ctx.rect(ctrPos.left + 1, ctrPos.top + 1, $width - 2, $heith - 2), ctx.stroke(), ctx.closePath();
      }
      ctx.restore();
    }
    ...
    if (null == this.selectedPoint) {
      ...
      if ('Top_Left' == this.selectedPoint) {
        var tlWidth = this.selectedSize.width - msg.dx,
            tlHeight = this.selectedSize.height - msg.dy,
            tlX = this.selectedLocation.x + msg.dx,
            tlY = this.selectedLocation.y + msg.dy;
        tlX < this.x + this.width && (this.x = tlX, this.width = tlWidth), tlY < this.y + this.height && (this.y = tlY, this.height = tlHeight);
      } else if ('Top_Center' == this.selectedPoint) {
        var tcHeight = this.selectedSize.height - msg.dy,
        ...
        var trWidth = this.selectedSize.width + msg.dx,
            trY = this.selectedLocation.y + msg.dy;
        trY < this.y + this.height && (this.y = trY, this.height = this.selectedSize.height - msg.dy), trWidth > 1 && (this.width = trWidth);
      } else if ('Middle_Left' == this.selectedPoint) {
        var mlWidth = this.selectedSize.width - msg.dx,
            mlX = this.selectedLocation.x + msg.dx;
        mlX < this.x + this.width && (this.x = mlX), mlWidth > 1 && (this.width = mlWidth);
      } else if ('Middle_Right' == this.selectedPoint) {
        var mrWidth = this.selectedSize.width + msg.dx;
        mrWidth > 1 && (this.width = mrWidth);
      } else if ('Bottom_Left' == this.selectedPoint) {
        var blWidth = this.selectedSize.width - msg.dx,
            blX = this.selectedLocation.x + msg.dx;
        blWidth > 1 && (this.x = blX, this.width = blWidth);
        var blHeight = this.selectedSize.height + msg.dy;
        blHeight > 1 && (this.height = blHeight);
      } else if ('Bottom_Center' == this.selectedPoint) {
        var bcHeight = this.selectedSize.height + msg.dy;
        bcHeight > 1 && (this.height = bcHeight);
      } else if ('Bottom_Right' == this.selectedPoint) {
        var brWidth = this.selectedSize.width + msg.dx;
        brWidth > 1 && (this.width = brWidth);
        var brHeight = this.selectedSize.height + msg.dy;
...
//# sourceMappingURL=_baseElement.js.map