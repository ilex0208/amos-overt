...
function createEagleEye(stageObj) {
  return {
     ...
     * @return base64 img
     */
    getImage: function getImage(_width, _height) {
      var bound = stageObj.getBound(),
          scalewidth = 1,
          // 缩放当前绘图的宽度 (1=100%, 0.5=50%, 2=200%, 依次类推)
      scaleheight = 1; // 缩放当前绘图的高度 (1=100%, 0.5=50%, 2=200%, etc.)
      this.exportCanvas.width = stageObj.canvas.width, this.exportCanvas.height = stageObj.canvas.height;
      if (null != _width && null != _height) {
        this.exportCanvas.width = _width, this.exportCanvas.height = _height, scalewidth = _width / bound.width, scaleheight = _height / bound.height;
      } else {
      ...
      if (stageObj.childs.length > 0) {
        ctx.save(), ctx.clearRect(0, 0, this.exportCanvas.width, this.exportCanvas.height), stageObj.childs.forEach(function (a) {
          if (1 == stageObj.visible) {
            stageObj.save(), stageObj.translateX = 0, stageObj.translateY = 0, stageObj.scaleX = 1, stageObj.scaleY = 1, ctx.scale(scalewidth, scaleheight);
            if (bound.left < 0) {
            ...
            stageObj.paintAll = true, stageObj.repaint(ctx), stageObj.paintAll = false, stageObj.restore();
          }
        }), ctx.restore();
      }
    ...
    getData: function getData(_width, _height) {
      /**
       * 创建translate
       *
      ...
      function createTranslate(gdStage) {
        var scW = gdStage.stage.canvas.width,
      ...
      var limitW = void 0,
          limitH = void 0;
      if (stageObj.childs.length > 0) {
        ctx.save(), ctx.clearRect(0, 0, this.canvas.width, this.canvas.height), stageObj.childs.forEach(function (childStage) {
          if (true === childStage.visible) {
            childStage.save(), childStage.centerAndZoom(null, null, ctx), childStage.repaint(ctx), childStage.restore();
          }
          // true == childStage.visible && (childStage.save(), childStage.centerAndZoom(null, null, ctx), childStage.repaint(ctx), childStage.restore());
        });
        var $translate = createTranslate(stageObj.childs[0]),
        ...
        limitW > 1 && (limitW = 1), limitH > 1 && (limitW = 1), trX *= limitW, trY *= limitH, bound.left < 0 && (trX -= Math.abs(bound.left) * (this.width / bound.width)), bound.top < 0 && (trY -= Math.abs(bound.top) * (this.height / bound.height)), ctx.save(), ctx.lineWidth = 1, ctx.strokeStyle = 'rgba(255,0,0,1)', ctx.strokeRect(-trX, -trY, ctx.canvas.width * limitW, ctx.canvas.height * limitH), ctx.restore();
        var l = null;
        try {
    ...
    paint: function paint() {
      if (null != this.eagleImageDatas) {
        var ctx = stageObj.graphics;
        ctx.save(), ctx.fillStyle = 'rgba(211,211,211,0.3)', ctx.fillRect(stageObj.canvas.width - this.canvas.width - 2 * this.hgap, stageObj.canvas.height - this.canvas.height - 1, stageObj.canvas.width - this.canvas.width, this.canvas.height + 1), ctx.fill(), ctx.save(), ctx.lineWidth = 1, ctx.strokeStyle = 'rgba(0,0,0,1)', ctx.rect(stageObj.canvas.width - this.canvas.width - 2 * this.hgap, stageObj.canvas.height - this.canvas.height - 1, stageObj.canvas.width - this.canvas.width, this.canvas.height + 1), ctx.stroke(), ctx.restore(), ctx.putImageData(this.eagleImageDatas, stageObj.canvas.width - this.canvas.width - this.hgap, stageObj.canvas.height - this.canvas.height), ctx.restore();
      } else {
        this.eagleImageDatas = this.getData(stageObj);
      }
    },
     ...
     */
    eventHandler: function eventHandler(eventType, eventPos, st) {
      var epX = eventPos.x,
          epY = eventPos.y;
      if (epX > st.canvas.width - this.canvas.width && epY > st.canvas.height - this.canvas.height) {
        ...
        if ('mousedrag' == eventType && st.childs.length > 0) {
          var epDx = eventPos.dx,
              epDy = eventPos.dy,
              bound = st.getBound(),
              w_x_w = this.canvas.width / st.childs[0].scaleX / bound.width,
              h_y_h = this.canvas.height / st.childs[0].scaleY / bound.height;
          st.childs[0].translateX = this.lastTranslateX - epDx / w_x_w, st.childs[0].translateY = this.lastTranslateY - epDy / h_y_h;
        }
      } else {
        console.log('other');
      }
...
function Stage($canvas) {
  ...
  function _getEventPosition(ev) {
    var evPos = Util.getEventPosition(ev),
        offPos = Util.getOffsetPosition(_self.canvas);
    return evPos.offsetLeft = evPos.pageX - offPos.left, evPos.offsetTop = evPos.pageY - offPos.top, evPos.x = evPos.offsetLeft, evPos.y = evPos.offsetTop, evPos.target = null, evPos;
  }
  function mouseOver(ev) {
    document.onselectstart = function () {
      return false;
    }, this.mouseOver = true;
    var evPos = _getEventPosition(ev);
    _self.dispatchEventToScenes('mouseover', evPos), _self.dispatchEvent('mouseover', evPos);
  }
  function mouseOut(ev) {
    ...
    var evPos = _getEventPosition(ev);
    _self.dispatchEventToScenes('mouseout', evPos), _self.dispatchEvent('mouseout', evPos), _self.needRepaint = 0 == _self.animate ? false : true;
  }
  function mouseDown(ev) {
    var evPos = _getEventPosition(ev);
    _self.mouseDown = true, _self.mouseDownX = evPos.x, _self.mouseDownY = evPos.y, _self.dispatchEventToScenes('mousedown', evPos), _self.dispatchEvent('mousedown', evPos);
  }
  function mouseUp(ev) {
    var evPos = _getEventPosition(ev);
    _self.dispatchEventToScenes('mouseup', evPos), _self.dispatchEvent('mouseup', evPos), _self.mouseDown = false, _self.needRepaint = 0 == _self.animate ? false : true;
  }
  function mouseMove(ev) {
    isMenuTimeOut && (window.clearTimeout(isMenuTimeOut), isMenuTimeOut = null), isContextMenu = false;
    var evPos = _getEventPosition(ev);
  ...
  function click(ev) {
    var evPos = _getEventPosition(ev);
    _self.dispatchEventToScenes('click', evPos), _self.dispatchEvent('click', evPos);
  }
  function dbClick(ev) {
    var evPos = _getEventPosition(ev);
    _self.dispatchEventToScenes('dbclick', evPos), _self.dispatchEvent('dbclick', evPos);
  }
  function mouseWheel(ev) {
    var evPos = _getEventPosition(ev);
    _self.dispatchEventToScenes('mousewheel', evPos), _self.dispatchEvent('mousewheel', evPos), null != _self.wheelZoom && (ev.preventDefault ? ev.preventDefault() : (ev = ev || window.event, ev.returnValue = false), 1 == _self.eagleEye.visible && _self.eagleEye.update());
  }
  /**
   * 添加事件
   *
  ...
  function addEvent(targetDom) {
    if (Util.isIE || !window.addEventListener) {
      targetDom.onmouseout = mouseOut, targetDom.onmouseover = mouseOver, targetDom.onmousedown = mouseDown, targetDom.onmouseup = mouseUp, targetDom.onmousemove = mouseMove, targetDom.onclick = click, targetDom.ondblclick = dbClick, targetDom.onmousewheel = mouseWheel, targetDom.touchstart = mouseDown, targetDom.touchmove = mouseMove, targetDom.touchend = mouseUp;
    } else {
      targetDom.addEventListener('mouseout', mouseOut), targetDom.addEventListener('mouseover', mouseOver), targetDom.addEventListener('mousedown', mouseDown), targetDom.addEventListener('mouseup', mouseUp), targetDom.addEventListener('mousemove', mouseMove), targetDom.addEventListener('click', click), targetDom.addEventListener('dblclick', dbClick);
      if (Util.isFirefox) {
  ...
  this.initialize = function (cans) {
    addEvent(cans), this.canvas = cans, this.graphics = cans.getContext('2d'), this.childs = [], this.frames = 24, this.messageBus = new Util.MessageBus(), this.eagleEye = createEagleEye(this), this.wheelZoom = null, this.mouseDownX = 0, this.mouseDownY = 0, this.mouseDown = false, this.mouseOver = false, this.needRepaint = true, this.serializedProperties = ['frames', 'wheelZoom'];
  ...
  this.remove = function (st) {
    ...
    for (var i = 0; i < this.childs.length; i++) {
      if (this.childs[i] === st) {
        st.stage = null, this.childs = this.childs.del(i);
        return this;
      }
    }
    return this;
  ...
  this.addEventListener = function (evType, fn) {
    ...
    return this.messageBus.subscribe(evType, callback), this;
    ...
    return this.messageBus.publish(evType, msg), this;
  };
  var events = 'click,dbclick,mousedown,mouseup,mouseover,mouseout,mousemove,mousedrag,mousewheel,touchstart,touchmove,touchend,keydown,keyup'.split(','),
      tempSelf = this;
  ...
  this.saveImageInfo = function (width, heigh) {
    var imgData = this.eagleEye.getImage(width, heigh),
        newPage = window.open('about:blank');
    return newPage.document.write('<img src=\'' + imgData + '\' alt=\'from canvas\'/>'), this;
  },
  /**
   * 保存成本地图片
   * @param {number} width 区域宽
  ...
  this.saveAsLocalImage = function (width, heigh) {
    var imgData = this.eagleEye.getImage(width, heigh);
    return imgData.replace('image/png', 'image/octet-stream'), window.location.href = imgData, this;
  }, this.paint = function () {
    if (null != this.canvas) {
      this.graphics.save();
    ...
    var _self = this;
    this.childs.forEach(function (st) {
      var cw = _w - _self.canvas.width / 2,
          ch = _h - _self.canvas.height / 2;
      st.translateX = -cw, st.translateY = -ch;
    });
    ...
    return this.childs.forEach(function (st) {
      var eb = st.getElementsBound();
      eb.left < bound.left && (bound.left = eb.left, bound.leftNode = eb.leftNode), eb.top < bound.top && (bound.top = eb.top, bound.topNode = eb.topNode), eb.right > bound.right && (bound.right = eb.right, bound.rightNode = eb.rightNode), eb.bottom > bound.bottom && (bound.bottom = eb.bottom, bound.bottomNode = eb.bottomNode);
    }), bound.width = bound.right - bound.left, bound.height = bound.bottom - bound.top, bound;
  },
  /**
   * 将所有元素转化为json
   */
  this.toJson = function () {
    ...
    return this.serializedProperties.forEach(function (spItem) {
      var spItemValue = _self[spItem];
      'string' == typeof spItemValue && (spItemValue = '"' + spItemValue + '"'), resultJson += '"' + spItem + '":' + spItemValue + ',';
    }), resultJson += '"childs":[', this.childs.forEach(function (stItem) {
   ...
   */
  function () {
    0 == _self.frames ? setTimeout(arguments.callee, 100) : _self.frames < 0 ? (_self.repaint(), setTimeout(arguments.callee, 1000 / -_self.frames)) : (_self.repaint(), setTimeout(arguments.callee, 1000 / _self.frames));
  }(),
  /**
   * 定时器
   */
  setTimeout(function () {
    _self.mousewheel(function (target) {
      var temp = null == target.wheelDelta ? target.detail : target.wheelDelta;
...
//# sourceMappingURL=_stage.js.map