...
function getLinksArray(node_a, node_b) {
  ...
  if (node_a && node_b) {
    var a_outlinks = node_a.outLinks;
    var b_inlinks = node_b.inLinks;
    // 循环遍历 node_a 中 outLinks 存储的每一项link
    if (a_outlinks && b_inlinks) {
      for (var i = 0; i < a_outlinks.length; i++) {
        var link1 = a_outlinks[i];
        for (var j = 0; j < b_inlinks.length; j++) {
          var link2 = b_inlinks[j];
          if (link1 === link2) {
...
function getAllLinks(node_a, node_b) {
  var a_bLinks = getLinksArray(node_a, node_b),
...
function getOtherLinksByLink(link) {
  var allLinks = getAllLinks(link.nodeA, link.nodeZ);
...
function Link(nodeA, nodeZ, text) {
  ...
  function getInterSection(nodeZ, nodeA) {
    var dline = Util.lineF(nodeZ.cx, nodeZ.cy, nodeA.cx, nodeA.cy),
  ...
  this.removeHandler = function () {
    ...
    var endPos = void 0;
    return null != this.arrowsRadius && (endPos = getInterSection(this.nodeZ, this.nodeA)), null == endPos && (endPos = {
      x: this.nodeZ.cx,
      y: this.nodeZ.cy
    }), endPos;
    ...
    var radian = Math.atan2(endPos.y - startPos.y, endPos.x - startPos.x),
        g = {
  ...
  this.paintPath = function (ctx, paths) {
    ...
    if (null != this.arrowsRadius) {
      var path_last_2 = paths[paths.length - 2],
  ...
  this.paintArrow = function (ctx, path_2, lastPath) {
    var arrowOff = this.arrowsOffset,
    ...
    ctx.beginPath(), ctx.fillStyle = 'rgba(' + this.strokeColor + ',' + this.alpha + ')', ctx.moveTo(movePos.x, movePos.y), ctx.lineTo(arrowX, arrowY), ctx.lineTo(arrowLineto.x, arrowLineto.y), ctx.stroke(), ctx.closePath();
  }, this.paint = function (ctx) {
    if (null != this.nodeA && null != !this.nodeZ) {
      var paths = this.getPath(this.nodeIndex);
      this.path = paths, ctx.strokeStyle = 'rgba(' + this.strokeColor + ',' + this.alpha + ')', ctx.lineWidth = this.lineWidth, this.paintPath(ctx, paths), paths && paths.length > 0 && this.paintText(ctx, paths);
    }
  };
  var _PI = -(Math.PI / 2 + Math.PI / 4);
  this.paintText = function (ctx, _path) {
    ...
    if (this.text && this.text.length > 0) {
      var _width = (path_last.x + path_0.x) / 2 + this.textOffsetX,
          _heigth = (path_last.y + path_0.y) / 2 + this.textOffsetY;
      ctx.save(), ctx.beginPath(), ctx.font = this.font;
    ...
    ctx.shadowBlur = 10, ctx.shadowColor = 'rgba(0,0,0,1)', ctx.shadowOffsetX = 0, ctx.shadowOffsetY = 0;
    ...
    for (var i = 1; i < this.path.length; i++) {
      var prevPath = this.path[i - 1],
          nextPath = this.path[i];
...
function FoldLink(nodeA, nodeZ, text) {
  this.initialize = function () {
  ...
  }, this.initialize(nodeA, nodeZ, text), this.getStartPosition = function () {
    var startPos = {
      x: this.nodeA.cx,
      y: this.nodeA.cy
    };
    return 'horizontal' == this.direction ? this.nodeZ.cx > startPos.x ? startPos.x += this.nodeA.width / 2 : startPos.x -= this.nodeA.width / 2 : this.nodeZ.cy > startPos.y ? startPos.y += this.nodeA.height / 2 : startPos.y -= this.nodeA.height / 2, startPos;
  }, this.getEndPosition = function () {
    var startPos = {
      x: this.nodeZ.cx,
      y: this.nodeZ.cy
    };
    return 'horizontal' == this.direction ? this.nodeA.cy < startPos.y ? startPos.y -= this.nodeZ.height / 2 : startPos.y += this.nodeZ.height / 2 : startPos.x = this.nodeA.cx < startPos.x ? this.nodeZ.x : this.nodeZ.x + this.nodeZ.width, startPos;
    ...
    if (this.text && this.text.length > 0) {
      ...
      var textW = ctx.measureText(this.text).width,
          defaultW = ctx.measureText('田').width;
      ctx.fillStyle = 'rgba(' + this.fontColor + ', ' + this.alpha + ')', ctx.fillText(this.text, _x - textW / 2, _y - defaultW / 2), ctx.stroke(), ctx.closePath(), ctx.restore();
    }
  };
}
...
function FlexionalLink(nodeA, nodeZ, text) {
  this.initialize = function () {
    ...
    return 'horizontal' == this.direction ? sp.x = this.nodeZ.cx < sp.x ? this.nodeA.x : this.nodeA.x + this.nodeA.width : sp.y = this.nodeZ.cy < sp.y ? this.nodeA.y : this.nodeA.y + this.nodeA.height, sp;
  }, this.getEndPosition = function () {
    var ep = {
      x: this.nodeZ.cx,
      y: this.nodeZ.cy
    };
    return 'horizontal' == this.direction ? ep.x = this.nodeA.cx < ep.x ? this.nodeZ.x : this.nodeZ.x + this.nodeZ.width : ep.y = this.nodeA.cy < ep.y ? this.nodeZ.y : this.nodeZ.y + this.nodeZ.height, ep;
  }, this.getPath = function (nodeIndex) {
...
function CurveLink(nodeA, nodeZ, text) {
  ...
  }, this.initialize(nodeA, nodeZ, text), this.paintPath = function (ctx, paths) {
    if (this.nodeA === this.nodeZ) {
      return void this.paintLoop(ctx);
    }
    ctx.beginPath(), ctx.moveTo(paths[0].x, paths[0].y);
    for (var i = 1; i < paths.length; i++) {
      var prevLink = paths[i - 1],
          nextLink = paths[i],
          distanceX = (prevLink.x + nextLink.x) / 2,
          distanceY = (prevLink.y + nextLink.y) / 2;
      distanceY += (nextLink.y - prevLink.y) / 2, ctx.strokeStyle = 'rgba(' + this.strokeColor + ',' + this.alpha + ')', ctx.lineWidth = this.lineWidth, ctx.moveTo(prevLink.x, prevLink.cy), ctx.quadraticCurveTo(distanceX, distanceY, nextLink.x, nextLink.y), ctx.stroke();
    }
    ctx.stroke();
    ctx.closePath();
    if (null != this.arrowsRadius) {
...
Link.prototype = new InteractiveElement(), FoldLink.prototype = new Link(), FlexionalLink.prototype = new Link(), CurveLink.prototype = new Link();
