 ...
 * @returns
 */
function changeColor(ctx, img, rgbR, rgbG, rgbB) {
  // 采用连续赋值,将img.width赋值给canvas,接着再将canvas.width赋值给临时变量tempW（此处不可拆开）
  let tmpW = canvas.width = img.width,
    tmpH = canvas.height = img.height;
  ...
  return alarmImageCache[img.src] = m, m;
}
 ...
 *
 * @param {any} image
 * @param {any} rgbR RGBA 中R取值 0至255
 * @returns
 */
function genImageAlarm(img, rgbR) {
  let _rgbR = rgbR || 255;// 默认红色255
  try {
    ...
    else
    {
      ...
      alarmImg.src = changeColor(graphics, img, _rgbR);
