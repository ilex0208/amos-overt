...
function rotatePoints(a, b, c) {
  ...
  for (let e = 0; e < b.length; e++) {
    let f = rotatePoint(a.x, a.y, b[e].x, b[e].y, c);
    d.push(f);
  }
  return d;
}
...
function toJson(a) {
  let b = 'backgroundColor,visible,mode,rotate,alpha,scaleX,scaleY,shadow,translateX,translateY,areaSelect,paintAll'.split(','),
...
function changeColor(a, b, c, d, e) {
  let f = canvas.width = b.width,
    g = canvas.height = b.height;
  a.clearRect(0, 0, canvas.width, canvas.height),
    a.drawImage(b, 0, 0);
  let h = a.getImageData(0, 0, b.width, b.height);
  let i = h.data;
  for (let j = 0; j < f; j++) {
    for (let k = 0; g > k; k++) {
      let l = 4 * (j + k * f);
