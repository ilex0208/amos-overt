...
Tortoise.prototype.forward = function(distance) {
  let $p = this.p,
    $w = this.w;
  return $p.x = $p.x + distance * $w.x,
    $p.y = $p.y + distance * $w.y,
...
Tortoise.prototype.resize = function(ratio) {
  let tempW = this.w;
  return tempW.x = tempW.x * ratio,
    tempW.y = tempW.y * ratio,
    this;
};
