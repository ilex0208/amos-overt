...
function InteractiveElement() {
  this.initialize = function() {
    InteractiveElement.prototype.initialize.apply(this, arguments),
      ...
      this.isMouseOver = !1;
    let a = 'dragable,selected,showSelected,isMouseOver'.split(',');
    this.serializedProperties = this.serializedProperties.concat(a);
  },
    this.initialize(),
    ...
    this.mousedragHandler = function(a) {
      let b = this.selectedLocation.x + a.dx,
...
function EditableElement() {
  ...
  let b = ['Top_Left', 'Top_Center', 'Top_Right', 'Middle_Left', 'Middle_Right', 'Bottom_Left', 'Bottom_Center', 'Bottom_Right'];
  this.paintCtrl = function(a) {
    if (0 != this.editAble) {
      a.save();
      for (let c = 0; c < b.length; c++) {
        let d = this.getCtrlPosition(b[c]);
        d.left -= this.cx,
          d.right -= this.cx,
          d.top -= this.cy,
          d.bottom -= this.cy;
    ...
    this.isInBound = function(a, c) {
      if (this.selectedPoint = null, 1 == this.editAble) {for (let e = 0; e < b.length; e++) {
        let f = this.getCtrlPosition(b[e]);
    ...
    this.mousedragHandler = function(a) {
      if (null == this.selectedPoint) {
        ...
        if ('Top_Left' == this.selectedPoint) {
          let d = this.selectedSize.width - a.dx,
