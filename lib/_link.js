...
function Link(b, c, g) {
  function h(b, c) {
    let d = Util.lineF(b.cx, b.cy, c.cx, c.cy),
      e = b.getBound(),
  ...
  this.initialize = function(b, c, d) {
    ...
    this.getPath = function() {
      ...
      let d = e(this.nodeA, this.nodeZ);
      if (1 == d) {return [b, c];}
      let f = Math.atan2(c.y - b.y, c.x - b.x),
        g = {
    ...
    this.paintPath = function(a, b) {
      ...
      for (let c = 1; c < b.length; c++) {null == this.dashedPattern ? a.lineTo(b[c].x, b[c].y) : a.AmostDashedLineTo(b[c - 1].x, b[c - 1].y, b[c].x, b[c].y, this.dashedPattern);}
    ...
    this.paint = function(a) {
      if (null != this.nodeA && null != !this.nodeZ) {
        let b = this.getPath(this.nodeIndex);
        this.path = b,
  ...
  this.paintText = function(ctx, _path) {
    let c = _path[0],
      d = _path[_path.length - 1];
    if (4 == _path.length && (c = _path[1], d = _path[2]), this.text && this.text.length > 0) {
    ...
    this.isInBound = function(b, c) {
      if (this.nodeA === this.nodeZ) {
        let d = this.bundleGap * (this.nodeIndex + 1) / 2,
...
function FoldLink(a, b, c) {
  this.initialize = function() {
    ...
    this.getPath = function(a) {
      ...
      let f, g, h = e(this.nodeA, this.nodeZ),
    ...
    this.paintText = function(a, b) {
      if (this.text && this.text.length > 0) {
        ...
        a.save(),
          a.beginPath(),
          a.font = this.font;
        let f = a.measureText(this.text).width,
