...
function Link(nodeA, nodeZ, text) {
  ...
  this.isInBound = function(_x, _y) {
    ...
    for (let i = 1; i < this.path.length; i++) {
      let temp = this.path[i - 1], i = this.path[i];
      if (1 == Util.isPointInLine( {x: _x,y: _y}, temp, i)) {
        falg = true;
        break;
      }
    }
...
function FoldLink(a, b, c) {
  ...
  this.getPath = function(a) {
    ...
    let f, g, h = getLinksSize(this.nodeA, this.nodeZ),
...
function FlexionalLink(a, b, c) {
  this.initialize = function() {
    ...
    this.getPath = function(a) {
      ...
      let d = [],
        f = getLinksSize(this.nodeA, this.nodeZ),
