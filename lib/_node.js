  ...
  this.paint = function(ctx) {
    if (this.image) {
      ...
      ctx.globalAlpha = this.alpha,
        null != this.image.alarm && null != this.alarm ? ctx.drawImage(this.image.alarm, -this.width / 2, -this.height / 2, this.width, this.height) : ctx.drawImage(this.image, -this.width / 2, -this.height / 2, this.width, this.height),
    ...
    if ('string' == typeof img) {
      ...
      null == image ? (image = new Image, image.src = img, image.onload = function() {
        imageCache[img] = image,
          1 == isSelfSize && _tempSelf.setSize(image.width, image.height);
        let f = Util.genImageAlarm(image);
        f && (image.alarm = f),
          _tempSelf.image = image;
      }) : (isSelfSize && this.setSize(image.width, image.height), this.image = image);
    } else {this.image = img,
      1 == isSelfSize && this.setSize(img.width, img.height);}
  },
...
function LinkNode(text, href, tar) {
  ...
  this.paint = function(ctx) {
    ctx.beginPath(),
      ...
      this.width = ctx.measureText(this.text).width,
      this.height = ctx.measureText('田').width,
      this.isVisited && null != this.visitedColor ? (ctx.strokeStyle = 'rgba(' + this.visitedColor + ', ' + this.alpha + ')', ctx.fillStyle = 'rgba(' + this.visitedColor + ', ' + this.alpha + ')') : (ctx.strokeStyle = 'rgba(' + this.fontColor + ', ' + this.alpha + ')', ctx.fillStyle = 'rgba(' + this.fontColor + ', ' + this.alpha + ')'),
      ctx.fillText(this.text, -this.width / 2, this.height / 2),
      this.isMouseOver && (ctx.moveTo(- this.width / 2, this.height), ctx.lineTo(this.width / 2, this.height), ctx.stroke()),
      ctx.closePath(),
      this.paintBorder(ctx),
      this.paintCtrl(ctx),
      this.paintAlarmText(ctx);
...
AbstractNode.prototype = new eleExt.EditableElement,
